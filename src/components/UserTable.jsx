const users = [
  ...
  },
  {
    id: 5,
    name: "Emma Brooks",
    email: "emma.brooks@northstar.io",
    role: "Manager",
    status: "Active",
  },
  {
    id: 6,
    name: "Olivia Diaz",
    email: "olivia.diaz@northstar.io",
    role: "Support",
    status: "Active",
  },
]
...
export default function UserTable({
  ...
  subtitle = "Monitor role assignment and account status",
  limit,
  showActions = true,
}) {
  const rows = typeof limit === "number" ? users.slice(0, limit) : users

  return (
    <section className="surface mt-6 overflow-hidden p-4 sm:p-6">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3 border-b pb-4">
        <div>
          <h2 className="title-font text-xl font-bold tracking-tight">{title}</h2>
          <p className="text-sm muted-text">{subtitle}</p>
        </div>
